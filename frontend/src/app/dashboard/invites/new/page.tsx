...
function NewInviteContent() {
  ...
  // Optional pre-selected debtId (from dashboard "Create invite" button on a specific debt)
  const preselectedDebtId = searchParams.get('debtId') || undefined

  // ── Form state ──────────────────────────────────────────────────────────
  const [privacyLevel,  setPrivacyLevel]  = useState<PrivacyLevel>('creditor_name')
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const result = await inviteApi.create({
      ...(preselectedDebtId ? { debtId: preselectedDebtId } : {}),
      privacyLevel,
    ...
    if (!result.ok) {
      setApiError(typeof result.error === 'string' ? result.error : JSON.stringify(result.error))
      return
    }
          ...
          <div className="w-full max-w-md">
            ...
            <div className="flex flex-col items-center mb-8">
              ...
              <p className="text-mid-gray text-sm mt-2 text-center">
                Share this with someone you trust · expires in {expiresInDays} days
              </p>
            </div>
        ...
        <div className="max-w-lg mx-auto">
          ...
          <div className="mb-8">
            ...
            <p className="text-mid-gray text-sm mt-2">
              Share this privately with someone you trust. They&apos;ll choose
              which bill to help with and pay the creditor directly.
            </p>
          </div>
